...
(function () {
    ...
    /* ==================== 钩子 ==================== */
    /* 一代引擎（krz.restate 魔改）在子类上重复实现了很多方法（Game_Actor 自有 gainHp、
       Game_Player 自有 isMapPassable，均已实测确认），只打基类 prototype 会被覆盖失效。
       hookMethod 沿原型链找到方法"实际定义层"打补丁；__rmhook 去重避免经多个子类重复包装。
       另注意：本引擎没有 isPlayer() 方法，玩家判断用实例身份（self === $gamePlayer）。 */
    function hookMethod(ctor, name, wrap) {
        try {
            ...
            if (orig.__rmhook) return false; // 已在其定义层打过（如 Game_CharacterBase 被多个子类走到）
    ...
    function hookAll() {
        ...
        /* 一击必杀（注意 subject 是方法必须调用；v7 误用 this.subject.isActor 从未生效） */
        ...
        /* 无敌/免消耗：逐类打，覆盖 Game_Actor 的引擎自有实现 */
        ...
        /* 移动：Game_Player 引擎层自有 isMapPassable；isPlayer 不存在，用实例身份 */
        var movers = [Game_Player, Game_CharacterBase];
        ...
        /* 锁定：本引擎 Game_Battler 没有 update 方法（v7 包装 undefined 每次调用必抛异常，
           锁定从未生效），改用定时回写，地图/战斗都生效 */
        ...
        var ModApp = defineComponent({
            ...
            setup: function () {
                ...
                function showPanel(on) {
                    show.value = on;
                    if (on) { refreshStatus(); refreshList(); }
            ...
            <n-config-provider :theme="darkTheme" :theme-overrides="themeOverrides" :locale="zhCN"
              ...
              <div v-if="show" class="rm-panel" style="pointer-events:auto;position:fixed;top:12px;right:12px;width:440px;max-width:92vw;height:90vh;display:flex;flex-direction:column;background:#0e1016;border:1px solid #2b3143;border-radius:14px;box-shadow:0 18px 48px rgba(0,0,0,.62),0 2px 10px rgba(0,0,0,.45);overflow:hidden;z-index:2147483002;transform:translate3d(0,0,0);will-change:transform;backface-visibility:hidden;">
                ...
                <div style="display:flex;align-items:center;gap:9px;padding:11px 14px;background:linear-gradient(135deg,#1c2434 0%,#161a23 60%);border-bottom:1px solid #272c39;">
                  ...
                  <span style="font-weight:600;color:#f2f4f9;font-size:14px;">再刷一把 MOD</span>
